...
public class CombatHandler : MonoBehaviour
{
    ...
    public InputCharacterController movementInput;
    ...
    [SerializeField] private ParticleHandler punchParticle;
    ...
    //Events
    public UnityEvent<EnemyBase> OnTrajectory;
    public UnityEvent<EnemyBase> OnHit;
    public UnityEvent<EnemyBase> OnCounterAttack;

    int animationCount = 0;

    string[] attacks;

    public HealthBarUi healthBarEnemy;
    ...
    void Update()
    {
        ...
        {
            healthBarEnemy.SetName(lockedTarget.enemyName);
            healthBarEnemy.UpdateUI(lockedTarget.health);
        }
    ...
    void AttackCheck()
    {
        ...
        if(lockedTarget !=null)
            lastHitFocusObject.transform.position = lockedTarget.transform.position;
    ...
    public void Attack(EnemyBase target, float distance)
    {
    ...
    void AttackType(string attackTrigger, float cooldown, EnemyBase target, float movementDuration)
    {
        ...
        {
            Time.timeScale = .5f;
            GKUtils.Log("LAST HIT", "in cor");
            //lastHitFocusObject.localPosition = lockedTarget.transform.position;
    ...
    void MoveTorwardsTarget(EnemyBase target, float duration)
    {
    ...
    float TargetDistance(EnemyBase target)
    {
    ...
    EnemyBase ClosestCounterEnemy()
    {
        ...
        {
            EnemyBase enemy = enemyManager.allEnemies[i].enemyScript;
    ...
    bool isLastHit()
    {
        GKUtils.Log("LAST HIT", "insde");

        if (lockedTarget == null)
            return false;


        bool v1 = lockedTarget.health <= lockedTarget.hitFactor;
        bool v = enemyManager.aliveEnemyCount<= 1 ;
        GKUtils.Log("LAST HIT", "still working " + v + $" lh { lockedTarget.gameObject.name} { lockedTarget.health} :: " + v1);
        return v && v1;
    }
